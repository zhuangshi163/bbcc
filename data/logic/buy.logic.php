...
//         $this->_order_data['store_gc_id_commis_rate_list'] = $store_gc_id_commis_rate_list;
    ...
    private function _createOrderStep4() {
        ...
        //分销商佣金模块
        $did = $_COOKIE['udid']?$_COOKIE['udid']:$_COOKIE['did'];
        if($did){
        	$model_member = Model('member');
        	$model_dian_profit = Model('dian_profit');
        	$did_info = $model_member->getMemberInfo(array('did'=>intval($did)),'did,l_id,l_b');
        	$dianresults = $model_member->getMemberList(array('did'=>array('in',implode(',', $did_info))),'did,wx_shop_name');
        }
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    //分销佣金处理
                    //array('1011'=>array('name'=>'电商哥','money'=>35),'1011'=>array('name'=>'电商哥','money'=>35),'1011'=>array('name'=>'电商哥','money'=>35))
                	if($dianresults){//$order_goods[$i]['goods_pay_price']
                        	$dians = array();
                        	$Dianfitdata = array();
                        	...
                        	$pdid_info = array();
                        	$pdid_info['a_ocnt'] = array('exp','a_ocnt+1'); //更新一级订单数
                        	
                        	$ppdid_info = array();
                        	$ppdid_info['b_ocnt'] = array('exp','b_ocnt+1'); //更新二级订单数
                        	
                        	$pppdid_info = array();
                        	$pppdid_info['c_ocnt'] = array('exp','c_ocnt+1'); //更新三级订单数
                        	...
                        	foreach ($dianresults as $k=>$v){
                        		if($v['did'] == $did){ //一级
                        			$dians[$v['did']]['money'] = floor($order_goods[$i]['goods_price'] * 0.1);
                        			$level = 1;                      			 
                        		}elseif ($v['did'] == $did_info['l_id']){ //二级
                        			$dians[$v['did']]['money'] = floor($order_goods[$i]['goods_price'] * 0.1);
                        			$level = 2; 
                        		}else { //三级
                        			$dians[$v['did']]['money'] = floor($order_goods[$i]['goods_price'] * 0.15);
                        			$level = 3; 
                        		}
                        		$Dianfitdata[] = array('did'=>$v['did'],'order_id'=>$order_goods[$i]['order_id'],'goods_id'=>$order_goods[$i]['goods_id'],'goods_price'=>$order_goods[$i]['goods_price'],'profit'=>$dians[$v['did']]['money'],'level'=>$level,'type'=>1,'state'=>0);
                        		
                        		$dians[$v['did']]['name'] = $v['wx_shop_name'];
                        
                        	}
                        	$order_goods[$i]['level_money'] = serialize($dians);
...
//                         	throw new Exception('订单保存失败[未生成商品数据]');
                            ...
                            $model_dian_profit->addDianProfits($Dianfitdata);
                        }
                    
                    //                 print_r($order_goods[$i]);
                    //                 echo $order_goods[$i]['goods_price'];
                    //                 print_r($dians);
                    //                 echo $order_goods[$i]['level_money'];
                    //                 throw new Exception('订单保存失败[未生成商品数据]');
                    $i++;
