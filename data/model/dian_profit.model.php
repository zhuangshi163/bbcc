...
class dian_profitModel extends Model {

    public function __construct(){
        parent::__construct('dian_profit');
    }
     ...
     * @return array
     */
    public function getDianProfitInfo($condition, $field = '*', $master = false) {
        return $this->table('dian_profit')->field($field)->where($condition)->master($master)->find();
    }
     ...
     * @return boolean
     */
    public function addDianProfits($data) {
    	 return $this->table('dian_profit')->insertAll($data);
    }    
    
    
    //($where,'SUM(point_goodsnum) as goodsnum')
    /**
     * 查询等级佣金
     */
    public function getLevelDianProfits($where, $field = '*', $group = '', $page = 0, $limit = 0,$order = '') {
    	return $this->table('dian_profit')->field($field)->where($where)->group($group)->page($page)->limit($limit)->order($order)->select();
    }
	...
	public function addMember($param) {
		...
		try {
		    ...
		    $member_info['language']		= $param['language'];
		    $member_info['city']		= $param['city'];
		    $member_info['province']		= $param['province'];
		    $member_info['country']		= $param['country'];
		    //添加邀请人(推荐人)会员积分 
