...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listOp() {
        ...
        if(intval($_POST['status'])){
        	$condition['order_state'] = intval($_POST['status']);
        }elseif (intval($_POST['refund_state'])){
...
//print_r($payment_list);
     ...
     * 订单列表
     */
    public function getDianOrderListOp() {
    	$model_order = Model('order');
    
    	//$dids = $this->member_info['did'].','.$this->member_info['l_id'].','.$this->member_info['l_b'];
    	$condition = array('did'=>$this->member_info['did'],'l_id'=>$this->member_info['did'],'l_b'=>$this->member_info['did'],'_op' => 'or');
    	$model_member = Model('member');
    	$did_list = $model_member->getMemberList($condition,'did');
    	$dids = array();
    	foreach ($did_list as $k=>$v){
    		$dids[] = $v['did'];
    	}
    	//$dids = array_column($did_list,'did');
    	
    	//print_r($dids);
    	$condition = array('outlet_id'=>array('in',implode(',', $dids)));
    	if(intval($_POST['status']) && $_POST['status'] != 'all'){
    		$condition['order_state'] = intval($_POST['status']);
    	}elseif (intval($_POST['refund_state'])){
    		$condition['refund_state'] = array('in','1,2');
    	...
    	foreach ($order_list_array as $value) {
    		...
    		foreach ($value['extend_order_goods'] as $k => $goods_info) {
    			...
    			$level_money_arr  = unserialize($goods_info['level_money']);
    			$level_money_sum = $level_money_sum + $level_money_arr[$this->member_info['did']]['money'];
    			$value['extend_order_goods'][$k]['level_money'] = $level_money_arr[$this->member_info['did']]['money'];
    			$value['extend_order_goods'][$k]['wx_shop_name'] = $level_money_arr[$this->member_info['did']]['name'];
    			
    		}
    
    		$value['level_money_sum'] = $level_money_sum;
    		$order_group_list[$value['pay_sn']]['order_list'][] = $value;
    		...
    		if($value['outlet_id'] == $this->member_info['did']){
    			$level = '一级订单';
    		}elseif ($value['outlet_id'] == $this->member_info['l_id']){
    			$level = '二级订单';
    		}else {
    			$level = '三级订单';
    		}
    		...
    		$order_group_list[$value['pay_sn']]['level'] = $level;
